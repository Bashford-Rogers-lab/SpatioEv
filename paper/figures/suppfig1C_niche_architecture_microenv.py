"""
Supplementary Figure 1C — Niche architecture & microenvironment feature trends
===============================================================================
8 niche-level features from the architecture_topology, microenvironment, and
other input blocks, shown as LOWESS trends vs quantile pseudotime.

Layout: 2 rows × 4 columns, colour-coded by sub-category:
  Architecture (blues)      — hull circularity, cell spacing, tortuosity
  Microenvironment (greens) — fibroblast, mesenchyme, endothelial proportions
  Boundary (oranges)        — boundary fraction, mean external degree

Run:
    python notebooks/suppfig1C_niche_architecture_microenv.py

Output: paper/notebooks/results/pseudotime_exp2/suppfig1C_niche_architecture_microenv.pdf  (.png)
"""
...
# Row 0 — architecture + 1 microenvironment
# Row 1 — microenvironment (cont.) + boundary
FEATURES = [
    # Row 0
    ("geometry__hull_circularity",               "Hull circularity",      "#1f78b4"),
    ("geometry__mean_nearest_neighbor_distance", "Mean cell spacing",     "#4393c3"),
    ("topology__skeleton_tortuosity",            "Duct tortuosity",       "#6baed6"),
    ("surround_prop__Fibroblasts",               "Fibroblast prop.",      "#33a02c"),
    # Row 1
    ("surround_prop__Vimentin_only_mesenchyme",  "Mesenchyme prop.",      "#74c476"),
    ("surround_prop__Endothelial_cells",         "Endothelial prop.",     "#a1d99b"),
    ("graph_boundary__boundary_fraction",        "Boundary fraction",     "#e6550d"),
    ("graph_boundary__mean_external_degree",     "Mean external degree",  "#fd8d3c"),
]

N_ROWS, N_COLS = 2, 4

CATEGORY_LEGEND = [
    mpatches.Patch(color="#1f78b4", label="Niche architecture"),
    mpatches.Patch(color="#33a02c", label="Microenvironment"),
    mpatches.Patch(color="#e6550d", label="Niche boundary"),
]
...
def make_figure():
    ...
    fig.subplots_adjust(left=0.08, right=0.98, top=0.95, bottom=0.19)

    # ── Save ─────────────────────────────────────────────────────────────────
    RESULT_DIR.mkdir(parents=True, exist_ok=True)
    out_pdf = RESULT_DIR / "suppfig1C_niche_architecture_microenv.pdf"
    out_png = RESULT_DIR / "suppfig1C_niche_architecture_microenv.png"
