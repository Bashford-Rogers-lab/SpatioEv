"""Organize and rewrite the ST_TMAG_E1_31 analysis assets."""
...
def build_adata(paths: dict[str, Path], marker_path: Path, project_dir: Path = PROJECT_DIR) -> Path:
    ...
    def merge_nuclear_rows_if_needed(table: pd.DataFrame) -> pd.DataFrame:
        if "nc_ratio" in table.columns:
            return table

        required = {"fov", "label", "mask_type", "area"}
        missing = required - set(table.columns)
        if missing:
            raise ValueError(f"Cannot compute nc_ratio; missing columns: {sorted(missing)}")

        whole = table.loc[table["mask_type"].eq("whole_cell")].copy()
        nuclear = table.loc[table["mask_type"].eq("nuclear")].copy()
        if whole.empty or nuclear.empty:
            raise ValueError("Expected both whole_cell and nuclear rows to compute nc_ratio.")

        merge_keys = ["fov", "label"]
        if "source_ark_wdir" in table.columns:
            merge_keys = ["source_ark_wdir", *merge_keys]

        nuclear_cols = [col for col in nuclear.columns if col not in merge_keys]
        nuclear = nuclear[merge_keys + nuclear_cols].rename(
            columns={col: f"{col}_nuclear" for col in nuclear_cols}
        )
        merged = whole.merge(nuclear, how="left", on=merge_keys, validate="one_to_one", sort=False)
        if "area_nuclear" not in merged.columns:
            raise ValueError("Nuclear merge did not produce area_nuclear.")

        whole_area = pd.to_numeric(merged["area"], errors="coerce")
        nuclear_area = pd.to_numeric(merged["area_nuclear"], errors="coerce")
        merged["nc_ratio"] = nuclear_area / whole_area.replace(0, np.nan)
        return merged

    arcsinh = merge_nuclear_rows_if_needed(arcsinh)
    size = merge_nuclear_rows_if_needed(size)
...
def adata_creation_notebook() -> list[nbf.NotebookNode]:
    return [
        ...
        md(
            ...
            `ark_wdir_1` contains FOV 1-9 and `ark_wdir_2` contains FOV 10-18.
        ...
        md(
            """
            ## 3b. Merge Nuclear Rows When Needed

            Some ARK exports store whole-cell and nuclear measurements as
            separate rows. When `nc_ratio` is absent, this notebook pairs nuclear
            rows to whole-cell rows by `source_ark_wdir + fov + label`, appends
            nuclear measurements with `_nuclear`, and computes
            `nc_ratio = area_nuclear / area`.
            """
        ),
        code(
            """
            def merge_nuclear_rows_if_needed(table):
                if "nc_ratio" in table.columns:
                    return table
            ...
            ark_arcsinh = merge_nuclear_rows_if_needed(ark_arcsinh)
            ark_size_normalized = merge_nuclear_rows_if_needed(ark_size_normalized)

            print("Prepared arcsinh:", ark_arcsinh.shape)
            print("Prepared size-normalized:", ark_size_normalized.shape)
            print("Columns added for nuclear metadata:", [c for c in ark_arcsinh.columns if c.endswith("_nuclear")][:10], "...")
            """
        ),
        md(
            """
...
def qc_notebook() -> list[nbf.NotebookNode]:
    return [
        ...
        md(
            ...
            - Reordered markers according to `ST_TMAG_E1_31_markers.csv`
        ...
        code(
            ...
            if list(adata.var_names) != marker_order:
                raise ValueError("adata.var_names do not match marker order from ST_TMAG_E1_31_markers.csv")
        ...
        md(
            ...
            `/Volumes/Shihong_3/ST_TMAG_E1_31/qc/segmentation_qc/adata/{SAMPLE_ID}_adata_segmentation_qc_filtered.h5ad`
...
def manual_gating_notebook() -> list[nbf.NotebookNode]:
    return [
        ...
        md(
            ...
            Fill or update the gate values in `ST_TMAG_E1_31_manual_gates.csv`.
        ...
        code('get_ipython().run_line_magic("gui", "qt")'),
        code(
            """
            def require_scimap():
                if sm is None:
        ...
        md(
            ...
            - `phenotyping/manual_gating/adata/ST_TMAG_E1_31_adata_scimap_rescaled.h5ad`
            - `phenotyping/manual_gating/adata/ST_TMAG_E1_31_adata_scimap_phenotyped.h5ad`
            - `phenotyping/manual_gating/tables/ST_TMAG_E1_31_phenotype_assignments.csv`
            """
        ),
    ]
