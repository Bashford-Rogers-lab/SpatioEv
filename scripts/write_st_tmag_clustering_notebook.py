"""Write the ST_TMAG_E1_95 FOV-aware clustering phenotyping notebook."""
...
def notebook_cells() -> list[nbf.NotebookNode]:
    return [
        ...
        md(
            ...
            By default this notebook uses the filtered object from segmentation
            QC. If that file is missing, it falls back to the unfiltered AnnData
            from the creation notebook.
            """
        ),
        code(
            f"""
            ...
            SELECTED_FOVS = ["fov2", "fov5", "fov7", "fov10", "fov12", "fov13", "fov16", "fov23", "fov24", "fov26", "fov28"]
            STRICT_SELECTED_FOVS = False
        ...
        md(
            ...
            The filtered AnnData should contain only QC-pass cells. Coordinates
            remain FOV-local pixel coordinates, so all clustering and static
            spatial review is grouped by FOV. If `SELECTED_FOVS` is set, the
            notebook subsets to those FOVs before marker validation and
            clustering.
            """
        ),
        code(
        ...
        md(
            ...
            `ST_TMAG_E1_95_markers.csv` is the channel-order source of truth for
        ...
        md(
            ...
            marker order follows `ST_TMAG_E1_95_markers.csv`.
            """
        ),
        code(
            """
        ...
        code(
            ...
            def plot_fov_spatial_grid(
                ...
                if categorical:
                    categories = sorted(adata_obj.obs[color_key].astype(str).dropna().unique())
        ...
        code(
            ...
            print(f"Analyzing all included QC-pass cells: {adata_work.n_obs:,}")
        ...
        md(
            ...
            If `ST_TMAG_E1_95_level0_cluster_annotation_reviewed.csv` exists, the
